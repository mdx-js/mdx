import {Chart} from './_component/snowfall.server.js'
export {Home as default} from './_component/home.server.js'
export const info = {
  author: [
    {name: 'John Otander', github: 'johno', twitter: '4lpine'}
  ],
  published: new Date('2017-12-23'),
  modified: new Date('2021-11-01')
}
export const year = 2018

{/* lint disable heading-style */}

Markdown for the\
**component era**
=================

MDX is an authorable format that lets you seamlessly write JSX in your markdown
documents.
You can import components, such as interactive charts or alerts, and embed them
within your content.
This makes writing long-form content with components a blast.
🚀

***

## What does it do?

<div className="home-columns">
  <div className="home-column-start">
    You write markdown with embedded components through JSX:

    ```mdx path="example.mdx"
    import {Chart} from './snowfall.js'
    export const year = 2018

    # Last year’s snowfall

    In {year}, the snowfall was above average.
    It was followed by a warm spring which caused
    flood conditions in many of the nearby rivers.

    <Chart year={year} color="#fcb32c" />
    ```
  </div>

  <div className="home-column-end">
    It’s compiled to JavaScript you can use in any framework that supports JSX:

    {/* lint disable */}

    <div className="home-preview">
      # Last year’s snowfall

      In {year}, the snowfall was above average.
      It was followed by a warm spring which caused flood conditions in many of
      the nearby rivers.

      <Chart
        year={year}
        color="#fcb32c"
      />
    </div>

    {/* lint enable */}
  </div>
</div>

[Try it out on our playground »][playground]

***

<div className="card big">
  ## New: MDX v2!

  Years of hard work have led to a much improved version 2.
  Here’s the highlights:

  <div className="emoji-list">
    *   🔁 **Improved syntax** makes it easier to use markdown in JSX, in
        markdown
    *   🧩 **Full JavaScript support** turns `{2 * Math.PI}` into {2 * Math.PI}
    *   🔌 **New integrations** for esbuild, Rollup, and Node.js
    *   🔥 **Support any JSX runtime**: React, Preact, Vue, Emotion, Ink, you
        name it
    *   🎒 **400% smaller** while having more features
    *   🏃‍♀️ at least **25% faster** while doing more work
    *   🧵 …and much, much, so much more
  </div>

  [Read the announcement »][v2]
</div>

## In short

<div className="emoji-list">
  *   ❤️ **Powerful**: MDX blends markdown and JSX syntax to fit perfectly in
      JSX-based projects
  *   💻 **Everything is a component**: Use existing components in your
      MDX and import other MDX files as components
  *   🔧 **Customizable**: Decide which component is rendered for each markdown
      construct (`{h1: MyHeading}`)
  *   📚 **Markdown-based**: The simplicity and elegance of markdown remains,
      you interleave JSX only when you want to
  *   🔥 **Blazingly blazing fast**: MDX has no runtime, all compilation occurs
      during the build stage
</div>

{/* To do: quotes. */}

> It’s extremely useful for using design system components to render markdown
> and weaving interactive components in with existing markdown.
>
> — [**@chrisbiscardi**][quote]

[quote]: https://twitter.com/chrisbiscardi/status/1022304288326864896

[playground]: /playground/

[v2]: /blog/v2/
