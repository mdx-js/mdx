...
It takes an object mapping component names to components, which MDX uses to
replace default components with custom components.
Take this example:
...
1. The first link (`#alpha`) is dotted and violet, because `a` is the HTML
   equivalent for the markdown syntax `[]()`.
2. The second link (`#bravo`) remains unchanged, because in JSX syntax `a` is a
   literal tag name, which [MDX does not modify][mdx-no-literals-modification].
3. The third link (`#charlie`) is dashed and tomato, because in this JSX syntax
   `Link` is a reference to a component.
...
[mdx-no-literals-modification]: https://github.com/mdx-js/mdx/issues/821
