...
For version 2, we worked hard so they’re no longer required and MDX can more
easily be used with other tools.
...
no longer require any framework-specific code to glue them together with MDX.
...
it happened, instead of a bundler like webpack complaining about an error in an
...
See [¶ Architecture in `@mdx-js/mdx`][architecture] for how our compiler works.
See [§ Extending MDX][extending] for several plugins that use the new tree to
enhance MDX, how to use plugins, and how to create them.
...
We’ve also published [`@types/mdx`][types-mdx], a types-only package that can be
used with any (community) integration.
...
Now we reach that number every week.
...
but over the years it also resulted in inconsistencies and duplicate content.
