...
Markdown often feels more natural to type than HTML or JSX for common elements
...
A few other features from JavaScript are supported in MDX as well: expressions in
...
JavaScript is **whitespace insensitive** and  **unforgiving** (it does crash on typos).
...
*   Autolinks do not work in MDX.
    The reason is that they can look similar to JSX (for example:
...
convenient to use components.
JSX is typically combined with a frontend framework like React, Preact, or Vue.
These frameworks add support for components. This helps you re-use common markup
...
It makes repeating markup more clear and allows for separation of concerns.
...
These ESM features can be used within MDX to import components or declare variables:
