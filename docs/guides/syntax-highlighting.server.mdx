...
For example, it’s possible to pass that string as a prop with a remark plugin:

```js path="remark-meta-as-attributes.js"
import {visit} from 'unist-util-visit'

/** @type {import('unified').Plugin<Array<void>, import('hast').Root>} */
function remarkMetaAsAttributes() {
  return (tree) => {
...
That can be achieved with the same remark plugin as above with a different
`onelement` handler:
