...
<Note type="important">
  **Important**: `rehype-highlight` requires the use of ESM and will not work with CJS.
  This means that if using `next.js`, you will need a `next.config.mjs` file instead of
  a `next.config.js` file for your [configuration](https://nextjs.org/docs/api-reference/next.config.js/introduction).  This applies to all `rehype` and also `remark` plugins.
</Note>
