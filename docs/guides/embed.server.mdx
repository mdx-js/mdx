...
runtime.
Doing it at compile time means a bit of extra effort is spent when compiling but
that readers will have a very fast experience.
Doing it at runtime gives some flexibility but depends on what framework you use
(so it’s specific to react, preact, vue, etc.) and moves all the work to your
users.
