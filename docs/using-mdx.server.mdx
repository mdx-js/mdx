...
It’s possible to use JavaScript expressions, inside curly braces, in MDX:
...
Instead of defining or importing data inside MDX files, data can also be passed
when the MDX content component is used.
This object with arbitrary data is available as `props`.
Take for example:
