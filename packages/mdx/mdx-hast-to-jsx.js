...
const {isEmptyObject, paramCase: camelCase} = require('@mdx-js/util')
...
const serializeChild = node => typeof node === 'string' ? node : generate(node).code
...
const buildElement = ({tagName, props, parentName, children = []}) => {
  ...
  return t.jsxElement(
    t.jsxOpeningElement(
      tagName,
      [
        buildParentNameAttribute(parentName),
        ...buildProps(props)
      ].filter(Boolean)
    ),
...
const buildJsx = ({children, layout, importNodes, exportNodes}) => {
  ...
  const jsx = transformSync(
    `
    ${importNodes.map(n => n.value).join('\n')}
    ${exportNodes.map(n => n.value).join('\n')}
